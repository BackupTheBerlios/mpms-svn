...
class dis_group_submit extends ksubmit{
	...
	function __construct(&$smarty){
		parent::__construct("dis_group", &$smarty);
	...
	function submited(&$inputs){
		$rez=null;
		try{
			$rez =&$this->query->execute("SELECT * FROM kaute.ed_group(".$inputs['index']->get_value().", false);");
		}
		catch(Exception $e){
		...
		$row = $rez->next();
		if($row[0] == 't'){	
			$this->log->notice("Group ".$inputs['kname']->get_value()." has been disabled.");
			return true;
		}
		else{
			$this->error = 2;
			$this->log->notice("Group ".$inputs['kname']->get_value()." could not been disabled. Reason unknown.");
		}
		return false;
	}
}
