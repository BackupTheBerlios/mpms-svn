...
class del_group_submit extends ksubmit{
	private $query;
	private $log;
	function __construct(&$smarty){
		parent::__construct("del_group", &$smarty);
	...
	function submited(&$inputs){
		$rez=null;
		try{
			$rez =&$this->query->execute("SELECT * FROM kaute.del_group(".$inputs['index']->get_value().")");
		}
		catch(Exception $e){
		...
		$row = $rez->next();
		if($row[0] == 't'){	
			$this->log->notice("Group ".$inputs['kname']->get_value()." has been deleted.");
			return true;
		}
		else{
			$this->error = 2;
			$this->log->notice("Group ".$inputs['kname']->get_value()." could not been deleted. Reason unknown.");
		}
		return false;
	}
}
...
class kdel_group{
	function main(){
		...
		$form =& new kform("kdel_group", &$smarty);
		...
		$form->add_submit(new del_group_submit(&$smarty));
		if($form->submited())
			$smarty->assign("mess", 1);
		$smarty->display('kdel_group_en.tpl');
	}
}

$kpage = new kdel_group();
$kpage->main();
?>
